...
import { fetchPriceEnquiry } from '@api/services/generalApi';
import { useDataFetching, useDebouncedSearch } from '@hooks';
import PriceEnquiryList from './PriceEnquiryList';
import { OverlayLoader } from '@components/Loader';

const PriceEnquiryScreen = ({ navigation }) => {

  const { data, loading, fetchData, fetchMoreData } = useDataFetching(fetchPriceEnquiry);
  ...
  const renderItem = ({ item }) => {
    ...
    return <PriceEnquiryList item={item} onPress={() => navigation.navigate('PriceEnquiryDetails', { id: item._id })} />;
  };

  const renderEmptyState = () => (
    <EmptyState imageSource={require('@assets/images/EmptyData/empty.png')} message={'No Price Enquiry Found'} />
  );
